...
const SB = ({ s }) => <span className={`b ${SM[s] || "bidle"}`}>{SL[s] || s}</span>;
const DB = ({ code }) => <span className={`b ${DEPT_CLS[code] || "bidle"}`}>{code}</span>;
...
function Prog({ pct, color = "cyan", h = 4 }) {
...
function GRow({ label, val, max, lw = 130 }) {
...
function AStrip({ type, text, time }) {
...
function Modal({ title, onClose, children, footer, zIndex }) {
    return <div className="mback" style={zIndex ? { zIndex } : {}} onClick={onClose}><div className="mod" onClick={e => e.stopPropagation()}><div className="mh"><span className="mtitle">{title}</span><button className="mclose" onClick={onClose}>×</button></div><div className="mbody">{children}</div>{footer && <div className="mfoot">{footer}</div>}</div></div>;
}
function Tabs({ tabs, active, onChange }) {
    return <div className="tabs">{tabs.map(t => <button key={t.id} className={`tab ${active === t.id ? "act" : ""}`} onClick={() => onChange(t.id)}>{t.label}</button>)}</div>;
}
function LiveBadge() {
