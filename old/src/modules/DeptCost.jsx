...
import { DEPARTMENTS, PROJECTS, WOS } from '../data/seed.js';
                        ...
                        <div className="card">
                            ...
                            <div className="cb">
                                <div className="g g3 mb16">
                                    <div><div className="tiny mb4">ACTIVE PROJECTS</div><div style={{ fontFamily: "var(--fd)", fontSize: 24, fontWeight: 800 }}>{PROJECTS.filter(p => p.dept === selDept.name && p.status === "active").length}</div></div>
                                    <div><div className="tiny mb4">WORK ORDERS MTD</div><div style={{ fontFamily: "var(--fd)", fontSize: 24, fontWeight: 800 }}>{WOS.filter(w => w.dept === selDept.name).length}</div></div>
                                ...
                                <div className="sep" />
                                <div className="tiny mb8">ACTIVE PROJECTS</div>
                                {PROJECTS.filter(p => p.dept === selDept.name).map(p => (
                ...
                <div>
                    ...
                    <div className="g g2">
                        <div className="card"><div className="ch"><span className="ct">Spend by Department</span></div><div className="cb">
                            {DEPARTMENTS.map(d => <div key={d.id} className="mb12"><div className="rowsb mb4"><span style={{ fontSize: 12 }}>{d.name}</span><span className="mono" style={{ color: d.color }}>AED {d.spent.toLocaleString()}</span></div><Prog pct={d.spent / d.budget * 100} color="cyan" h={6} /></div>)}
                        </div></div>
            ...
            {tab === "utilisation" && (
                <div className="g g2">
                    <div className="card"><div className="ch"><span className="ct">Printer Hours by Department (MTD)</span></div><div className="cb">
                        {DEPARTMENTS.map(d => {
                            const hrs = [22, 41, 18, 38, 28][DEPARTMENTS.indexOf(d)];
                    ...
                    <div className="card"><div className="ch"><span className="ct">Jobs by Technology per Dept</span></div><div className="cb">
                        <div className="tw"><table><thead><tr><th>Dept</th><th>FDM</th><th>SLA</th><th>SLS</th><th>Total</th></tr></thead><tbody>
                            {DEPARTMENTS.map(d => {
                                const fdm = WOS.filter(w => w.dept === d.name && w.tech === "FDM").length;
                                const sla = WOS.filter(w => w.dept === d.name && w.tech === "SLA").length;
                                const sls = WOS.filter(w => w.dept === d.name && w.tech === "SLS").length;
