                    ...
                    <div className="frow mb12" style={{ alignItems: "flex-start", gap: 16 }}>
                        <div>
                            ...
                            <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 8 }}>
                                ...
                                />
                                <label htmlFor="imageUpload" className="btn btg bts" style={{ fontSize: 10, cursor: "pointer", textAlign: "center" }}>
...
/* ── custom hook for simulating live print % ── */
function useLivePrintPct(project) {
    const [pct, setPct] = useState(project.printPct || 0);
    useEffect(() => {
        if (project.stage !== "printing" || pct >= 100) return;
        const id = setInterval(() => {
            setPct(p => Math.min(p + (Math.random() * 0.5), 100));
        }, 2000);
...
export function PrintRequests({ lcProjects, onLcProjectsChange, toast }) {
    ...
    const [editModal, setEditModal] = useState(null); // project id
    ...
    function saveEdit() {
        ...
        onLcProjectsChange(lcProjects.map(p => {
            if (p.id !== editModal) return p;
            const now = new Date();
            const ts = now.toLocaleString("en-GB", { day: "numeric", month: "short", hour: "2-digit", minute: "2-digit" });
            return {
                ...p,
                requestNote: editNotes,
                extraInfo: editExtraInfo,
                history: [{ stage: "edit", done: true, time: ts, note: "Request information updated." }, ...p.history]
            };
        }));
        toast("Request information updated", "s");
        setEditModal(null);
        setEditNotes("");
        setEditExtraInfo("");
    }
        ...
        <div>
            ...
            <div className="rowsb mb12">
                <div style={{ display: "flex", gap: 6, alignItems: "center" }}>
                    <button className={`btn bts ${viewMode === "kanban" ? "btp" : "btg"}`} onClick={() => setViewMode("kanban")}>📋 Kanban</button>
                    <button className={`btn bts ${viewMode === "list" ? "btp" : "btg"}`} onClick={() => setViewMode("list")}>📄 List</button>
                </div>
                        ...
                        return (
                            <div key={stage.id} style={{ background: "var(--bg3)", borderRadius: "var(--r3)", border: "1px solid var(--border)", display: "flex", flexDirection: "column", maxHeight: "calc(100vh - 200px)", overflow: "hidden" }}>
                                ...
                                <div style={{ flex: 1, overflowY: "auto", padding: 8, display: "flex", flexDirection: "column", gap: 8 }}>
                                    ...
                                    {cards.map(p => (
                                        <div key={p.id} className={`req-card ${p.dept.toLowerCase()}`} style={{ cursor: "pointer", marginBottom: 0, padding: 10, borderLeftWidth: 3, position: "relative" }}>
                                            ...
                                            <div style={{ fontFamily: "var(--fd)", fontSize: 12, fontWeight: 700, marginBottom: 3, lineHeight: 1.3, cursor: "pointer" }} onClick={() => setSel(p.id)}>{p.name}</div>
            ...
            {viewMode === "list" && (
                <div className="g g2">
                    {filtered.map(p => {
                        ...
                        return (
                            <div key={p.id} className={`req-card ${p.dept.toLowerCase()}`} style={{ cursor: "default" }}>
                                ...
                                <div style={{ fontFamily: "var(--fd)", fontSize: 13, fontWeight: 700, marginBottom: 4, cursor: "pointer" }} onClick={() => setSel(p.id)}>{p.name}</div>
